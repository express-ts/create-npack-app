...
function tryGitCommit(appPath) {
  try {
    ...
    execSync('git commit -m "Initialize project using Create Npack App"', {
      stdio: 'ignore',
    });
    return true;
  } catch (e) {
...
module.exports = function (
  ...
  if (useYarn) {
    ...
    dev = ['install', '--save-dev', verbose && '--verbose'].filter(e => e);
  }
  ...
  // TODO: deprecate 'devDependencies' key directly on templateJson
  ...
  // which doesn't install react and react-dom along with npack-scripts
  if (!isExpressInstalled(appPackage)) {
    args = args.concat([
      /*'express', 'react-dom'*/
    ]);
  }

  // Install template dependencies & devDependencies.
  if ((!isExpressInstalled(appPackage) || templateName) && args.length > 1) {
    try {
      Object.entries({
        dependencies: args,
        devDependencies: dev,
  ...
  if (
    dev.find(arg => arg.includes('typescript')) ||
...
function isExpressInstalled(appPackage) {
  const dependencies = appPackage.dependencies || {};

  return typeof dependencies.express !== 'undefined';
}
