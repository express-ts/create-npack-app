...
function verifyTypeScriptSetup() {
  ...
  const compilerOptions = {
    ...
    module: {
      parsedValue: ts.ModuleKind.CommonJS,
      value: 'commonjs',
    ...
    emitDecoratorMetadata: { value: true, reason: 'to use Decorators' },
    experimentalDecorators: { value: true, reason: 'to use Decorators' },
  ...
  // Reference `npack-scripts` types
  if (!fs.existsSync(paths.appTypeDeclarations)) {
    fs.writeFileSync(
      paths.appTypeDeclarations,
      `/// <reference types="npack-scripts" />${os.EOL}`
    );
  }
}
