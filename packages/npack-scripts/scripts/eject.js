...
console.log(
  chalk.cyan.bold(
    'NOTE: Create Npack App 2+ supports TypeScript, Sass, CSS Modules and more without ejecting: ' +
      'https://reactjs.org/blog/2018/10/01/create-npack-app-v2.html'
  )
);
console.log();
...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put npack-scripts in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove npack-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove npack-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"npack-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove npack-scripts and npack-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      ...
      if (process.platform === 'win32') {
        ...
        // Yarn is diligent about cleaning up after itself, but this causes the npack-scripts.cmd file
